...
class RacePlan {
    // Race Start
    setRaceStart(){
        race.checkpoints[0].target.arrival = RaceTime.timeToMinutes(racestartInput.value);
    ...
    getTargetArrival() {
        for (let i = 1; i < race.checkpoints.length; i++){
            ...
            cpB.target.arrival = cpA.target.arrival + cpB.target.split
        }
    }
    ...
    updateInputSection() {
        // race start input
        let a = race.checkpoints[0]?.target.arrival;
        racestartInput.value = a ? RaceTime.minutesToTime(a) : "";
